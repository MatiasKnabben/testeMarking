        ...
        <h3>Tipo do Pokémon</h3>
            ...
            <hr>
                <router-link :to="`/filtro/${selected}`" exact> <!-- Router Link para a pagina de filtro carregando um parâmetro que é o campo selecionado no form-select -->
                    <b-button id="botao" variant="outline-primary" class="mb-2" :bind="verificaBotao()" :disabled="this.habilitaBotao">
...
<script >
    ...
    data(){
        return{
            ...
            selected: null,
            //Váriavel para armazenar o valor do "disabled" do botao
            habilitaBotao: false
        }
    },
    mounted() {
    ...
    methods: {
        verificaBotao(){ //Metodo para verificar se o campo selecionado no filtro é null ou não
            if(this.selected == null){ //Se for null Desabilita o botao
                this.habilita = true;
            }
            else{
                this.habilita = false;  //Se não, habilita
            }
        }
    }
    })
